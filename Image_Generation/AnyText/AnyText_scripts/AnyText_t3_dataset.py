...
def draw_glyph2(font, text, polygon, vertAng=10, scale=1, width=512, height=512, add_space=True, font_color='white'): # 增加字体颜色选项，默认为白色(255, 255, 255, 255)或者'white'或者'#ffffff'。
    ...
    img = Image.new("RGBA", size=(width*scale, height*scale,), color=(0,0,0,0))

    # infer font size
    # image4ratio = Image.new("RGBA", img.size, "white")
    image4ratio = Image.new("RGBA", img.size, (0,0,0,0))
    draw = ImageDraw.Draw(image4ratio, 'RGBA')
    ...
    # print('\033[93m', font_size, '\n', type(font_size), '\033[0m')
    if not font_size > 0:
    ...
    if not vert:
        ...
        draw.text((rect[0][0]-text_width//2, rect[0][1]-text_height//2-top), text, font=new_font, fill=font_color) # 字体可改颜色
    else:
        x_s = min(box[:, 0]) + _w//2 - text_height//2
        y_s = min(box[:, 1])
        for c in text:
            # draw.text((x_s, y_s), c, font=new_font, fill=(255, 255, 255, 255))
            draw.text((x_s, y_s), c, font=new_font, fill=font_color) # 字体可改颜色
    ...
    pil_img = img
    # from ...Image_Process.Common_Image_Process.nodes import transparent_specify_color_pil
    # if font_color != 'black' or font_color != 000000:
    #     pil_img = transparent_specify_color_pil(pil_img, [0,0,0]) # 替换字体图中黑色背景为透明像素
    # import time
    # pil_img.save(os.path.join(os.path.expanduser('~'), 'Desktop', 'test', f'AnyText_font_img_{time.time()}.png'))
